...
export async function crawl(flags, pgConf, checkpointFn) {
    ...
    async function createCrawlEntry() {
        return db.insert({
            table: 'crawl',
            returning: 'id',
            data: {
    ...
    if (FLAGS.crawlName) {
        ...
        if (crawlExists && FLAGS.resumeIfAble) {
            ...
            await db.postgres.query('UPDATE crawl SET crawler_hostname=$1, crawler_ip=$2 WHERE id=$3', [os.hostname(), await publicIpv4(), CRAWL_ID]);
        }
        else {
    ...
    let [overallTimeout, overallTimeoutId] = createAsyncTimeout('Overall crawl timeout reached', CRAWL_TIMEOUT);
    try {
        let _crawlLoop = (async () => {
            ...
            for (let i = crawlListStartingIndex; i < crawlList.length; i++) {
                ...
                let [urlTimeout, urlTimeoutId] = createAsyncTimeout(`${url}: overall site timeout reached`, SITE_TIMEOUT);
...
async function exportResultCSV() {
    try {
        ...
        const query = `
          SELECT parent_page_url, screenshot
          FROM ad
          WHERE crawl_id = $1;
      `;
        // Exécuter la requête
        ...
        const shuffledRows = shuffleArray(res.rows);
        // Convertir les résultats en format CSV
        const csvContent = shuffledRows.map(row => `${row.parent_page_url},${row.screenshot}`).join("\n");
        // Chemin du fichier CSV
        const filePath = "./export.csv";
        // Vérifier si le fichier existe déjà
        const fileExists = fs.existsSync(filePath);
        // Si le fichier n'existe pas, ajouter les en-têtes
        if (!fileExists) {
            const headers = "Link,ScreenShot\n";
            fs.writeFileSync(filePath, headers, "utf8");
        }
        // Ajouter les nouvelles lignes au fichier CSV
        fs.appendFileSync(filePath, csvContent + "\n", "utf8");
        console.log(`Données ajoutées avec succès dans ${filePath}`);
    }
    catch (error) {
    ...
    try {
        ...
        const captureThirdPartyRequests = async (request) => {
            try {
                ...
                log.warning(`${page.url()}: Error handling intercepted request: ${e.message}`);
                request.continue(undefined, 0);
            }
        };
        ...
        if (e instanceof Error) {
            ...
            await db.updatePage(pageId, { error: e });
        }
        throw e;
    }
}
...
export async function launchBrowser(flags) {
    ...
    let chromeArgs = ['--disable-dev-shm-usage', '--disable-features=UseOzonePlatform', '--no-sandbox'];
