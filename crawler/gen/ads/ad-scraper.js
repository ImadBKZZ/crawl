        ...
        for (let adHandle of adHandles) {
            ...
            try {
                if (FLAGS.scrapeOptions.clickAds !== 'noClick') {
                    ...
                    await clickAd(adHandle.clickTarget, page, adId, metadata.parentPageId, metadata.crawlListIndex, metadata.originalUrl);
                }
            }
            catch (e) {
...
export async function scrapeAd(ad, page, metadata) {
    const db = DbClient.getInstance();
    let [timeout, timeoutId] = createAsyncTimeout(`${page.url()}: timed out while scraping ad`, AD_SCRAPE_TIMEOUT);
    ...
    const _crawlAd = (async () => {
        try {
            ...
            // Scrape ad content
            const adContent = await scrapeAdContent(page, ad, fullAdsDir, FLAGS.scrapeOptions.screenshotAdsWithContext, adId);
    ...
    try {
        ...
        if (adId) {
            db.postgres.query('DELETE FROM ad WHERE id=$1', [adId]);
            const dir = path.join(FLAGS.outputDir, await getCrawlOutputDirectory(), 'scraped_ads/ad_' + adId);
...
*/
async function scrapeAdContent(page, ad, screenshotDir, withContext, adId) {
