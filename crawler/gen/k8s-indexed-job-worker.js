...
(function (WorkerState) {
    WorkerState[WorkerState["PRE_CRAWL"] = 0] = "PRE_CRAWL";
    WorkerState[WorkerState["CRAWLING"] = 1] = "CRAWLING";
    WorkerState[WorkerState["POST_CRAWL"] = 2] = "POST_CRAWL";
...
async function validateCrawlSpec(input) {
...
async function main() {
    try {
        ...
        async function saveProfile() {
            ...
            let filterInvalidFiles = async (src, dst) => {
        ...
        if (flags.profileOptions.writeProfile && !(error instanceof NonRetryableError)) {
            ...
            if (shouldCheckpoint) {
                ...
                if (crawlIdQuery.rowCount == 0) {
                    ...
                    await db.postgres.query(`UPDATE crawl SET last_checkpoint_index=crawl_list_current_index WHERE id=$1`, [crawlIdQuery.rows[0].id]);
...
main();
//# sourceMappingURL=k8s-indexed-job-worker.js.map