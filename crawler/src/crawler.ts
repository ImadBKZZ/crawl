...
async function exportResultCSV(): Promise<void> {
  try {
      ...
      // Exécuter la requête
      const res = await db.postgres.query<Row>(query, [CRAWL_ID]);
...
async function AnalyserAds(): Promise<{ result: number }[]> {
  try {
      const db = DbClient.getInstance();
      ...
      // Exécuter la requête
      const res = await db.postgres.query<{ html: string }>(query, [CRAWL_ID]);
...
export async function launchBrowser(flags: CrawlerFlags) {
  ...
  let chromeArgs: string[] = ['--disable-dev-shm-usage','--disable-features=UseOzonePlatform', '--no-sandbox'];
