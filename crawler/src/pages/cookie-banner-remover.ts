...
// Interface pour représenter un bouton
interface Button {
    text: string | null;
    className: string;
    tag: string;
...
export async function removeCookieBanners(page: Page): Promise<void> {
    try {
        ...
        const extractButtonsFromFrame = async (frame: Frame): Promise<Button[]> => {
        ...
        let buttons: Button[] = await extractButtonsFromFrame(page.mainFrame());

        // Extraire les boutons de chaque iframe
        const frames = page.frames();
        for (const frame of frames) {
            try {
                const iframeButtons: Button[] = await extractButtonsFromFrame(frame);
